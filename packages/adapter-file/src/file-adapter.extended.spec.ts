...
describe('FileAdapter - Extended Tests', () => {
  ...
  const createMockRecord = (severity: 'DEBUG' | 'INFO' | 'WARNING' | 'ERROR' | 'CRITICAL' = 'INFO', message = 'Test message'): LoggerRecord => ({
    type: 'log',
    context: {},
    runtime: {
      severity,
  ...
  describe('edge cases and error handling', () => {
    it('should throw error for empty file path', () => {
      expect(() => new FileAdapter({ filepath: '' })).toThrow('filepath is required')
    })
    ...
    it('should handle special characters in file path', () => {
      const specialPath = resolve(testDir, 'test with spaces & special chars!.log')
  ...
  describe('encoding tests', () => {
    ...
    it('should handle unicode characters with utf-8', () => {
      const adapter = new FileAdapter({ filepath: testFilePath, encoding: 'utf-8' })
      const record = createMockRecord('INFO', '日本語テスト 🚀 emoji test ñáéíóú')
      ...
      const content = readFileSync(testFilePath, 'utf-8')
      const parsedRecord = JSON.parse(content.trim()) as LoggerRecord
      expect(parsedRecord.runtime.lines[0]?.message).toBe('日本語テスト 🚀 emoji test ñáéíóú')
    })
  })
  ...
  describe('append mode tests', () => {
    it('should overwrite file when append is false', () => {
      ...
      expect((JSON.parse(lines[0]!) as LoggerRecord).runtime.lines[0]?.message).toBe('Second message')
    })
  ...
  describe('complex record structures', () => {
    it('should handle records with nested payload structures', () => {
      ...
      expect(parsedRecord.runtime.lines[0]?.payload).toEqual(complexRecord.runtime.lines[0]?.payload)
